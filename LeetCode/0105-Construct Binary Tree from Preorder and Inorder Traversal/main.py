...
1. preorder traversal 的顺序是：根-左-右，因此可以通过preorder的第一个元素得到tree的root节点
...
拿着这两个数据，回到前序数组，我们就能把后序数组分为三个部分：
...
class Solution:
    ...
    def helper(self, preorder, pLeft, pRight, inorder, iLeft, iRight):
        ...
        node.left = self.helper(preorder, pLeft + 1, pLeft + len_left_tree, 
                                inorder, iLeft, idx - 1)
        node.right = self.helper(preorder, pLeft + len_left_tree + 1, pRight, 
                                inorder, idx + 1, iRight)
